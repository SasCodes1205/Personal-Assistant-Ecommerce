...
import { Check, X, Edit3, ShieldAlert, ShieldCheck, Shield } from 'lucide-react';
import { formatDistanceToNow } from 'date-fns';

// Compliance zone badge (Master Plan Section 6)
  ...
  if (zone === 'YELLOW')
    return (
      <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded bg-amber-50 text-amber-800 border border-amber-200">
        <Shield size={12} /> Yellow — claim check
      </span>
    );
  if (zone === 'RED')
    return (
      <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded bg-red-50 text-red-700 border border-red-200">
        <ShieldAlert size={12} /> Red — blocked
      </span>
    );
  return (
    <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded bg-gray-50 text-gray-500 border border-gray-200">
      Not reviewed
    </span>
  );
}
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="text-sm text-[var(--ink-muted)] mt-1">
          {drafts.length} pending · approving marks it ready; you send from Outlook Drafts
        </p>
      </header>
      ...
      {loading ? (
        ...
        <ul className="space-y-4">
          {drafts.map((d) => {
            const flags = d.complianceFlags?.flags ?? [];
            return (
              <li key={d.id} className="rounded-md border border-[var(--border)] bg-white">
                <header className="border-b border-[var(--border)] p-4 flex justify-between items-start">
                  <div>
                    ...
                    <div className="text-xs text-[var(--ink-muted)] mt-1 flex items-center gap-2 flex-wrap">
                      <span>To: {d.email.from}</span>
                      <span className="font-mono">{d.email.category}</span>
                      <span className="font-mono">{d.email.businessUnit}</span>
                      <ZoneBadge zone={d.complianceZone} />
                ...
                {flags.length > 0 && (
                  <div className="px-4 pt-3">
                    <div className="text-xs uppercase tracking-wide text-[var(--ink-muted)] mb-1">
                      Compliance notes
                    </div>
                    <ul className="text-xs space-y-1">
                      {flags.map((f: any, i: number) => (
                        <li key={i} className="text-amber-800">
                          • <span className="font-medium">{f.issue}</span> — {f.rule} ({f.severity})
                        </li>
                      ))}
                    </ul>
                  </div>
